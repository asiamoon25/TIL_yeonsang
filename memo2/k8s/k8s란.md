...
8. 인그레스(Ingress) : k8s 클러스터에서 외부에서 들어오는 요청을 클러스터 내의 서비스로 라우팅하는 규칙을 관리하는 API 오브젝트임. 인그레스를 통해 HTTP 와 HTTPS 트래픽을 특정 서비스로 전달할 수 있으며, URL 기반의 라우팅, 가상 호스팅, SSL/TLS 암호화, 로드 밸런싱 등의 기능을 제공함. 즉, 인그레스는 클러스터 외부에서 클러스터 내부의 특정 서비스로 트래픽을 안내하는 역할을 함.
	인그레스 리소스를 사용하려면, 인그레스 컨트롤러가 필요함.
	인그레서 컨트롤러는 인그레스 리소스에 정의된 규칙에 따라 트래픽을 처리하는 실제 구현체임. k8s 는 기본적으로 인그레서 컨트롤러를 포함하지 않으므로, NGINX Ingress Controller, Tracefik, HAProxy Ingress 등과 같은 인그레스 컨틀롤러를 별도로 설치해야함.

	* URL 기반 라우팅: 동일한 IP 주소에서 호스팅되는 여러 웹 애플리케이션으로 트래픽을 라우팅할 수 있음. 예를 들어 'example.com/frontend' 는 프론트엔드 서비스로, 'example.com/backend' 는 백엔드 서비스로 라우팅 할 수 있음.
	* 가상 호스팅: 여러 도메인 이름을 처리하고 각 도메인을 다른 서비스로 라우팅할 수 있음.
	* SSL/TLS 암호화 : 인그레스를 사용하여 클러스터로 들어오는 트래픽에 SSL/TLS 암호화를 적용할 수 있음. 인그레스를 통해 TLS 인증서를 관리하고, HTTPS 트래픽을 처리할 수 있음.
	* 로드 밸런싱 : 인그레스는 클러스터로 들어오는 트래픽의 로드 밸런싱을 지원함.요청을 여러 서비스 인스턴스에 분산시킬 수 있음.