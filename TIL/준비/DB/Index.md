...
1. **단일 컬럼 인덱스** : 하나의 컬럼을 기준으로 생성된 인덱스
   
2. **복합 컬럼 인덱스** : 두 개 이상의 컬럼을 결합하여 생성된 인덱스임. 검색이 여러 컬럼에 걸쳐 이루어질 때 유용함.
   
3. **유니크 인덱스** : 컬럼의 값이 고유해야 하는 경우 사용함. 중복된 값을 방지하고 데이터 무결성을 유지하는 데 도움이 됨.
   
4. **Full-text Index** (MySql) : 텍스트 데이터에 대한 전문 검색을 지원하는 인덱스, 문서나 긴 텍스트에서 키워드 검색을 빠르게 수행할 수 있음.
